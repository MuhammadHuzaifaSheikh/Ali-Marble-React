...
class Croper extends PureComponent {
    state = {
        src: null,
        crop: {
            unit: '%',
    ...
    onSelectFile = e => {
        ...
        if (e.target.files && e.target.files.length > 0) {
            ...
            this.setState({fileName: e.target.files[0].name, onViwe: false})
        }
    ...
    render() {
        ...
        return (
            <div className="App">
                ...
                {croppedImageUrl && (
                    <img alt="Crop" style={{width: '40%', height: '40%', display: 'inlineBlock'}}
                         src={croppedImageUrl}/>
                )}
            </div>
        );
